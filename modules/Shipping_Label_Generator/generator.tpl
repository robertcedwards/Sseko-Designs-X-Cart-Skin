{*
$Id: generator.tpl,v 1.13 2009/09/09 08:12:04 igoryan Exp $
...
<tr{cycle values=", class='TableSubHead'"}>
  ...
  <td align="center">{$v.order.login}</td>
  ...
  <td align="center">
    ...
    {foreach from=$v.labels item=label key=labelid}
      ...
      {else}
        <a href="{$current_location}/slabel.php?orderid={$orderid}{if $labelid ne ""}&amp;labelid={$labelid}{/if}">{$lng.lbl_download}</a><br />
      {/if}
    {/foreach}
    {else}
...
{if $have_ups_orders || $have_img_orders}
<tr>
  <td colspan="6">
    <hr />
  </td>
