{*
$Id: giftcert.tpl,v 1.12 2009/04/18 06:33:05 max Exp $
...
{if $login}
  ...
  {capture name=dialog}
    ...
    {if $smarty.get.gcid && $gc_array eq ""}
    ...
    <table cellspacing="1" class="data-table giftcert-table" summary="{$lng.lbl_gift_certificate_details|escape}">
      ...
      <tr>
        ...
        <td>
          ...
          {if $min_gc_amount gt 0 || $max_gc_amount gt 0}
            {$lng.lbl_gc_amount_msg}
            {if $min_gc_amount gt 0}
              {$lng.lbl_gc_from} {$config.General.currency_symbol}{$min_gc_amount|formatprice}
            {/if}
            {if $max_gc_amount gt 0}
              {$lng.lbl_gc_through} {$config.General.currency_symbol}{$max_gc_amount|formatprice}
            {/if}
          {/if}
        </td>
      </tr>
      ...
      {if $config.Gift_Certificates.enablePostMailGC eq "Y"}
          ...
          <td>
            <select id="recipient_country" name="recipient_country" size="1" onchange="javascript: check_zip_code_field(this, this.form.recipient_zipcode);">
              {foreach from=$countries item=c}
                <option value="{$c.country_code|escape}"{if $giftcert.recipient_country eq $c.country_code || ($c.country_code eq $config.General.default_country && $giftcert.recipient_country eq "") || ($c.country_code eq $userinfo.b_country && $giftcert.recipient_country eq "")} selected="selected"{/if}>{$c.country}</option>
              {/foreach}
            </select>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td class="data-required">*</td>
          <td>
            <input type="text" name="recipient_zipcode" size="30" value="{$giftcert.recipient_zipcode|escape}" onchange="javascript: check_zip_code_field(this.form.recipient_country, this.form.recipient_zipcode);" />
          </td>
        </tr>
        ...
        <td>
          <select name="gc_template">
            {foreach from=$gc_templates item=gc_tpl}
              <option value="{$gc_tpl|escape}"{if $gc_tpl eq $giftcert.tpl_file || $giftcert.tpl_file eq "" && $gc_tpl eq $config.Gift_Certificates.default_giftcert_template} selected="selected"{/if}>{$gc_tpl}</option>
            {/foreach}
          </select>
      ...
      <tr>
        ...
        <td class="buttons-row buttons-auto-separator">
          ...
          {if $smarty.get.gcindex ne ""}
            ...
            {if $active_modules.Wishlist ne "" && $action eq "wl"}
          ...
          {else}
            ...
            {if $active_modules.Wishlist && $login}
