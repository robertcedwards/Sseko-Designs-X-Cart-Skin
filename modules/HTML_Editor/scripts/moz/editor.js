/***********************************************************
InnovaStudio WYSIWYG Editor 2.9.8
Copyright © 2003-2007, INNOVA STUDIO (www.InnovaStudio.com). All rights reserved.
************************************************************/
...
function RENDER(sPreloadHTML)
  ...
  if(this.mode=="XHTML"){this.btnXHTMLFullSource=true;this.btnXHTMLSource=false;}
    ...
    sHTML+="<table id=idArea"+this.oName+" name=idArea"+this.oName+" border='0px' "+
            ...
            "<tr><td colspan=2 style=\"padding:0px;padding-left:1px;border:#cfcfcf 1px solid;background:url('"+this.scriptPath.substring(0, this.scriptPath.indexOf("moz/"))+"icons/bg.gif')\">"+
  ...
  document.write("<iframe style='width:1px;height:1px;overflow:auto;border:0px' id=\"myStyle"+this.oName+"\" name=\"myStyle"+this.oName+"\" src='"+this.scriptPath.substring(0, this.scriptPath.indexOf("moz/"))+"blank.gif'></iframe>");
...
function loadHTML(sHTML)//hanya utk first load.
    ...
    //RealTime
    //oEditor.addEventListener("keydown", new Function("editorDoc_onkeydown("+this.oName+")"), true) ;
    oEditor.addEventListener("keyup", new Function("editorDoc_onkeyup("+this.oName+")"), true);
    oEditor.addEventListener("mouseup", new Function("editorDoc_onmouseup("+this.oName+")"), true);

    oEditor.addEventListener("keydown", new Function("var e=arguments[0];doKeyPress(eval(e), "+this.oName+")"), false);
...
function insertLink(url,title,target)
    ...
    range = oSel.getRangeAt(0);
        ...
        if(oEl) {
			if(isset(target) && target!="")oEl.target=target;
        }
    }
...
function writeIconToggle(id,command,img,title)
    ...
    w=this.iconWidth;
    h=this.iconHeight;
    imgPath=this.scriptPath.substring(0, this.scriptPath.indexOf("moz/"))+this.iconPath+img;
    sHTML=""+
    ...
    "</span></td>";
    sHTML="<table align=left cellpadding=0 cellspacing=0 style='table-layout:fixed;width:"+w+"px'><tr>"+sHTML+"</tr></table>";
...
function writeBreakSpace()
    ...
    w=this.iconWidth;
    h=this.iconHeight;
    imgPath=this.scriptPath.substring(0, this.scriptPath.indexOf("moz/"))+this.iconPath+"brkspace.gif";
    sHTML=""+
    ...
    sHTML="<table align=left cellpadding=0 cellspacing=0 style='table-layout:fixed;width:"+5+"px'><tr>"+sHTML+"</tr></table>";
    return sHTML;
    }
function writeDropDown(id,command,img,title,width)
    ...
    /*** Localization ***/
    imgPath=this.scriptPath.substring(0, this.scriptPath.indexOf("moz/"))+this.iconPath+oUtil.langDir+"/"+img;
    /*** /Localization ***/
