{*
$Id: wishlists.tpl,v 1.14 2009/04/30 06:37:23 igoryan Exp $
...
<tr{cycle name=c1 values=', class="TableSubHead"'}>
  <td>{if $usertype eq "A" or ($usertype eq "P" and $active_modules.Simple_Mode ne "")}<a href="user_modify.php?user={$v.login|escape:"url"}&amp;usertype=C">{$v.firstname}&nbsp;{$v.lastname}&nbsp;({$v.login})</a>{else}{$v.firstname}&nbsp;{$v.lastname}&nbsp;({$v.login}){/if}</td>
  <td>
    <a href="wishlists.php?mode=wishlist&amp;customer={$v.login}">{$lng.lbl_n_items_in_wishlist|substitute:"items":$v.pcounts[0]}</a>
