{*
$Id: modify_return.tpl,v 1.22 2009/09/01 12:07:04 igoryan Exp $
...
<tr>
  ...
  <td>
    ...
    <select name="posted_data[returned_amount]"{if $return.status ne "A" && $return.status ne "C"} disabled="disabled"{/if}>
...
<tr>
  <td>&nbsp;</td>
  <td><input type="submit"{if $return.readonly eq "Y"} disabled="disabled"{/if} value="{$lng.lbl_modify|strip_tags:false|escape}" />{if $return.status eq 'A' || $return.status eq 'C'}&nbsp;&nbsp;&nbsp;<input type="button" value="{$lng.lbl_print_return_slip|strip_tags:false|escape}" onclick="javascript: window.open('returns.php?mode=print&amp;returnid={$return.returnid}','PRINT_RETURN_SLIP','width=350,height=300,toolbar=no,status=no,scrollbars=yes,resizable=no,menubar=yes,location=no,direction=no')" />{/if}</td>
</tr>
</table>
</form>
{/capture}
