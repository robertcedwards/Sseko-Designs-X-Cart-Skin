{*
$Id: returns.tpl,v 1.41 2009/06/26 06:33:16 joy Exp $
...
<tr>
  ...
  <td>{html_select_date prefix="start_date_" time=$search_prefilled.start_date start_year=$config.Company.start_year end_year=$config.Company.end_year}</td>
</tr>
<tr>
    <td>{$lng.lbl_period_to}</td>
    <td>{html_select_date prefix="end_date_" time=$search_prefilled.end_date start_year=$config.Company.start_year end_year=$config.Company.end_year}</td> 
</tr>
<tr>
...
{/section}
  ...
  <td align="center">
    ...
    <select name="update[{$v.returnid}][returned_amount]"{if $v.status ne "A" && $v.status ne "C"} disabled="disabled"{/if}>
