{*
$Id: manufacturers.tpl,v 1.57 2009/06/26 10:48:07 joy Exp $
...
var requiredFields = [
  ['manufacturer', "{$lng.lbl_manufacturer|strip_tags|wm_remove|escape:javascript}", false]{if $config.SEO.clean_urls_enabled eq "Y" && $administrate}, ['clean_url', "{$lng.lbl_clean_url|strip_tags|wm_remove|escape:javascript}", false]{/if}
]
-->
</script>
...
checkboxes = [{foreach from=$manufacturers item=v key=k}{if $k gt 0},{/if}'{if !(!$administrate and ($v.provider ne $login or $v.used_by_others))}to_delete[{$v.manufacturerid}]{/if}'{/foreach}];
...
<tr{cycle values=", class='TableSubHead'"}>
  <td align="center"><input type="checkbox" name="to_delete[{$v.manufacturerid}]"{if !$administrate and ($v.provider ne $login or $v.used_by_others)} disabled="disabled"{/if} /></td>
  ...
  <td align="center"><input type="text" name="records[{$v.manufacturerid}][orderby]" size="5" value="{$v.orderby}"{if !$administrate} disabled="disabled"{/if} /></td>
  <td align="center"><input type="checkbox" name="records[{$v.manufacturerid}][avail]" value="Y"{if $v.avail eq "Y"} checked="checked"{/if}{if !$administrate} disabled="disabled"{/if} /></td>
</tr>
...
{if !$administrate and $login ne $manufacturer.provider and $smarty.get.mode ne "add"}
{assign var="disabled" value=' disabled="disabled"'}
<br />
<font class="ErrorMessage">{$lng.txt_manufacturer_edit_warning}</font>
<br />

{elseif !$administrate and $manufacturer.used_by_others}
<br />
...
<form action="manufacturers.php" method="post" enctype="multipart/form-data" name="manufacturerform" onsubmit="javascript: return checkRequired(requiredFields){if $config.SEO.clean_urls_enabled eq "Y" && $administrate} &amp;&amp;checkCleanUrl(document.manufacturerform.clean_url){/if};">
...
{if $administrate && $mode eq "manufacturer_info" && $manufacturer.manufacturerid ne ''}
  <br />
