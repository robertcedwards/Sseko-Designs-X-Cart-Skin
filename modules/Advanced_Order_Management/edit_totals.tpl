{*
$Id: edit_totals.tpl,v 1.31 2009/07/23 07:35:40 igoryan Exp $
...
<tr{cycle values=', class="TableSubHead"'}> 
  ...
  <option value="{$shipping[ship_num].shippingid}"{if $shipping[ship_num].shippingid eq $cart_order.shippingid} selected="selected"{/if}>{$shipping[ship_num].shipping|trademark:$insert_trademark:"alt"}{if $config.Appearance.display_shipping_cost eq "Y"} ({include file="currency.tpl" value=$shipping[ship_num].rate plain_text_message=1}){/if}</option>
  {/section}
  </select>
  {else}
  ...
  <td>{$orig_order.shipping|trademark:$insert_trademark|default:$lng.lbl_aom_shipmethod_notavail}</td>
</tr>
...
<tr{cycle values=', class="TableSubHead"'}>
  ...
  <td>{include file="currency.tpl" value=$cart_order.display_subtotal}</td>
  <td>{include file="currency.tpl" value=$orig_order.display_subtotal}</td>
</tr>

<tr{cycle values=', class="TableSubHead"' advance=false}>
  <td>{$lng.lbl_discount}</td>
  <td>{include file="currency.tpl" value=$cart_order.discount}</td>
  <td rowspan="2">{include file="currency.tpl" value=$orig_order.discount}{if $orig_order.discount gt 0 and $orig_order.extra.discount_info.discount_type eq "percent"} ({$orig_order.extra.discount_info.discount}%){/if}</td>
</tr>

<tr{cycle values=', class="TableSubHead"'}>
  <td>{$lng.lbl_aom_new_discount}</td>
  <td><input type="checkbox" name="total_details[use_discount_alt]" onclick="javascript: MarkElement('discount')"{if $cart_order.use_discount_alt eq 'Y' && $cart_order.extra.discount_info.discount gt 0} checked="checked"{/if} />
<input type="text" size="12" maxlength="12" name="total_details[discount_alt]" value="{$cart_order.extra.discount_info.discount|escape}"{if $cart_order.use_discount_alt ne 'Y' || $cart_order.extra.discount_info.discount eq 0} disabled="disabled"{/if} />
...
<tr{cycle values=', class="TableSubHead"' advance=false}>
  ...
  <td>
{include file="currency.tpl" value=$cart_order.coupon_discount} (
<select name="total_details[coupon_alt]"{if $cart_order.use_coupon_discount_alt eq 'Y' && !$cart_order.use_old_coupon_discount} disabled="disabled"{/if}>
  <option value="">{$lng.lbl_none}</option>
{foreach from=$coupons item=v}
  <option value="{if $v.__deleted}__old_coupon__{else}{$v.coupon|escape}{/if}"{if $cart_order.coupon eq $v.coupon || ($cart_order.use_coupon_discount_alt eq 'Y' && !$cart_order.use_old_coupon_discount && $cart_order.__last_coupon eq $v.coupon)} selected="selected"{/if}>{$v.coupon} -{$v.discount}{if $v.coupon_type eq 'percent'}%{else}{$config.General.currency_symbol}{/if}{if $v.__deleted && $v.coupon_type neq "free_ship"} ({$lng.lbl_aom_coupon_not_found}){/if}</option>
{/foreach}
</select>
)
  </td>
  <td>{include file="currency.tpl" value=$orig_order.coupon_discount}{if $orig_order.coupon ne ""} ({$orig_order.coupon}){/if}</td>
</tr>
<tr{cycle values=', class="TableSubHead"'}>
  <td>{$lng.lbl_new_coupon_saving}</td>
  <td><input type="checkbox" name="total_details[use_coupon_discount_alt]" onclick="javascript: MarkElement('coupon_discount')"{if $cart_order.use_coupon_discount_alt eq 'Y' && !$cart_order.use_old_coupon_discount} checked="checked"{/if} />
  <input type="text" size="12" maxlength="12" id="coupon_discount_alt" name="total_details[coupon_discount_alt]" value="{$cart_order.coupon_discount|escape}"{if $cart_order.use_coupon_discount_alt ne 'Y' || $cart_order.use_old_coupon_discount} disabled="disabled"{/if} /></td>
  <td>&nbsp;</td>
</tr>
...
<tr{cycle values=', class="TableSubHead"'}>
  ...
  <td>{include file="currency.tpl" value=$cart_order.display_discounted_subtotal}</td>
  <td>{include file="currency.tpl" value=$orig_order.display_discounted_subtotal}</td>
</tr>
{/if}
...
{include file="currency.tpl" value=0}&nbsp;({$lng.lbl_free_ship_coupon_record|substitute:"code":$orig_order.coupon})
{else}
{include file="currency.tpl" value=$orig_order.display_shipping_cost}
{/if}
  </td>
</tr>
...
{/foreach}
  ...
  <td>
{foreach key=tax_name item=tax from=$cart_order.taxes}
{include file="currency.tpl" value=$tax.tax_cost}<br />
{/foreach}
  </td>
  <td nowrap="nowrap">
{foreach key=tax_name item=tax from=$orig_order.applied_taxes}
{$tax.tax_display_name}{if $tax.rate_type eq "%"} {$tax.rate_value}%{/if}: {include file="currency.tpl" value=$tax.tax_cost}<br />
{/foreach}
  </td>
</tr>
{/if}
...
<tr{cycle values=', class="TableSubHead"'}>
  ...
  <td>{include file="currency.tpl" value=$cart_order.payment_surcharge}</td>
  <td>{include file="currency.tpl" value=$orig_order.payment_surcharge}</td>
</tr>
{/if}
...
<tr{cycle values=', class="TableSubHead"'}>
  ...
  <td>{include file="currency.tpl" value=$cart_order.giftcert_discount}</td>
  <td>{include file="currency.tpl" value=$orig_order.giftcert_discount}</td>
</tr>
{/if}

<tr{cycle values=', class="TableSubHead"'}>
  <td><b style="text-transform: uppercase;">{$lng.lbl_total}</b></td>
  <td><b>{include file="currency.tpl" value=$cart_order.total}</b></td>
  <td><b>{include file="currency.tpl" value=$orig_order.total}</b></td>
</tr>
...
<tr{cycle values=' class="TableSubHead,"'}>
  ...
  <td>{include file="currency.tpl" value=$cart_order.applied_giftcerts[gc].giftcert_cost}</td>
  <td>{include file="currency.tpl" value=$orig_order.applied_giftcerts[gc].giftcert_cost}</td>
</tr>
{/section}
{/if}
