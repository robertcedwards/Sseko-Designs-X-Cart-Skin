{*
$Id: edit_products.tpl,v 1.31 2009/09/07 07:43:45 igoryan Exp $
...
<tr class="TableSubHead">
  ...
  <td valign="top">
    ...
    {else}
      ...
      <br />{$lng.lbl_aom_catalog_price}: {if $product.is_deleted ne "Y"}{include file="currency.tpl" value=$product.catalog_price}{else}{$lng.txt_not_available}{/if}
    {/if}
  </td>
  <td valign="top">{if $product.new}{else}{include file="currency.tpl" value=$orig_product.price}{/if}</td>
</tr>
<tr>
  <td valign="top">{$lng.lbl_aom_quantity_items}
    ...
    {if $config.General.unlimited_products ne "Y" and $product.is_deleted ne "Y" and $order.status ne "F" and $order.status ne "D" and $order.status ne "I" and not ($active_modules.Egoods && $product.distribution ne '')}
      <br /><br />
      <table cellpadding="0" cellspacing="0">
        <tr>
  ...
  <td valign="top">
    {if $product.deleted || ($active_modules.Egoods && $product.distribution ne '')}
      {$product.amount}
    {else}
