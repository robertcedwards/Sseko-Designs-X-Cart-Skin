{*
$Id: order_edit.tpl,v 1.19 2009/05/20 10:56:46 igoryan Exp $
...
<ul class="AomActions">
  ...
  <li class="AomAction{if !$single_mode and $has_giftcerts}Disabled{elseif $show eq "products"}Active{/if}">{if $show neq "products" and not (!$single_mode and $has_giftcerts)}<a href="order.php?orderid={$orderid}&amp;mode=edit&amp;show=products">{/if}{$lng.lbl_aom_edit_ordered_products}{if $show neq "products" and not (!$single_mode and $has_giftcerts)}</a>{/if}{if !$single_mode and $has_giftcerts} {$lng.lbl_aom_products_cannot_be_added}{/if}</li>
{if $has_giftcerts}
...
{/capture}
{include file="dialog.tpl" title=$lng.lbl_advanced_order_management content=$smarty.capture.dialog extra='width="100%"'}
<br />
{if $show eq "preview"}
