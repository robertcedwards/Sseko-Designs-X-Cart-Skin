{*
$Id: debug_templates.tpl,v 1.30 2009/04/18 06:33:00 max Exp $
  ...
  try {
    if (_smarty_console) {
      ...
      _smarty_console.document.write(
        ...
        + '<style type="text/css">' + "\n"
        + ' html, body { font-size: 12px; padding: 0px; margin: 0px; } ' + "\n"
...
{if $t.type eq "template" && $webmaster_mode eq "editor"}
        + '<a hr'+'ef="{$catalogs.admin}/file_edit.php?{$XCARTSESSNAME}={$XCARTSESSID}&amp;file=%2f{$t.filename|escape:url}&amp;opener=console" target="_blank" onmouseover="javascript: if (window.mainWnd && mainWnd.tmo) mainWnd.tmo(\'{$t.filename|replace:"/":"0"}\', this);">{$t.filename}<'+'/a>'
{else}
        + '{$t.filename}'
{/if}
