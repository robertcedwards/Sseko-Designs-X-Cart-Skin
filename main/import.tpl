{*
$Id: import.tpl,v 1.23.2.1 2010/01/19 11:28:17 serg Exp $
...
{$lng.txt_import_data_note|substitute:"IMPORT_MANUAL":$import_manual}

<br /><br />

{if $need_select_provider && $providers}
...
{foreach from=$providers item=p}
  <option value="{$p.login|escape}"{if $data_provider eq $p.login} selected="selected"{/if}>{$p.firstname}{if $p.lastname} {$p.lastname}{/if} ({$p.login})</option>
{/foreach}
</select>
