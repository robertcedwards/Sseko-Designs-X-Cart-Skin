{*
$Id: export.tpl,v 1.32 2009/10/05 08:15:40 igoryan Exp $
...
<tr>
  ...
  <td>
    ...
    {section name=prov loop=$providers}
      <option value="{$providers[prov].login}" {if $export_data.provider eq $providers[prov].login}selected{/if}>{$providers[prov].login} ({$providers[prov].title} {$providers[prov].lastname} {$providers[prov].firstname})</option>
    {/section}
    {/if}
    </select>
  </td>
