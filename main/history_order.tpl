{*
$Id: history_order.tpl,v 1.126.2.7 2010/01/25 12:02:51 joy Exp $
...
{/if}
{if ($usertype eq "A" || ($usertype eq "P" && $active_modules.Simple_Mode)) && $active_modules.Advanced_Order_Management}
...
{/if}
<td class="ButtonsRowRight">{include file="buttons/button.tpl" button_title=$lng.lbl_print_invoice target="_blank" href="order.php?orderid=`$order.orderid`&mode=invoice" substyle="link"}</td>
...
<script type="text/javascript" src="{$SkinDir}/main/history_order.js"></script>
...
{elseif $order.capture_enable}
  ...
  {include file="currency.tpl" value=$order.init_total assign=init_total plain_text_message=Y}

  {if !$order.capture_noninit || $order.capture_limit}
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  {if $order.capture_noninit}
    ...
    <table cellspacing="0" cellpadding="2">
      <tr>
        ...
        <td>{include file="currency.tpl" value=$order.init_total}</td>
      </tr>
      <tr>
        <td>{$lng.lbl_current_total}:</td>
        <td>{include file="currency.tpl" value=$order.capture_full_total}</td>
      </tr>
    </table>
...
<tr>
  <td><input type="submit" value="{$lng.lbl_apply_changes|strip_tags:false|escape}" onclick="javascript: return submitOrder(this);" /></td>
  <td>
{if $usertype eq "A"}
...
{if $usertype eq "A" && (not $active_modules.Google_Checkout or not $order.extra.goid) && $order.charge_info and ($order.charge_info.refunded_total gt 0 or $order.charge_info.refund_avail gt 0)}
    ...
    <table cellspacing="1" cellpadding="3">
      {if $order.charge_info.refunded_total gt 0}
        <tr>
          ...
          <td>{include file="currency.tpl" value=$order.charge_info.refunded_total}</td>
        </tr>
      {/if}
...
{if $active_modules.RMA ne '' && (($usertype eq 'A' && $current_membership_flag ne 'FS') || ($usertype eq 'P' && $active_modules.Simple_Mode))}
