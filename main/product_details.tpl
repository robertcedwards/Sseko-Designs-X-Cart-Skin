{*
$Id: product_details.tpl,v 1.90 2009/09/14 08:28:25 igoryan Exp $
...
{section name=prov loop=$providers}
    <option value="{$providers[prov].login}"{if $product.provider eq $providers[prov].login} selected="selected"{/if}>{$providers[prov].login} ({$providers[prov].title} {$providers[prov].lastname} {$providers[prov].firstname})</option>
{/section}
  </select>
...
<tr>
  ...
  <select name="categoryids[]" class="InputWidth" multiple="multiple" size="8" style="width: 70%;">
...
{if $active_modules.Manufacturers ne "" and !$is_pconf}
<tr>
...
<tr> 
  ...
  <select name="forsale">
    ...
    <option value="N"{if $product.forsale ne "Y" && $product.forsale ne "" && $product.forsale ne "H" && ($product.forsale ne "B" || not $active_modules.Product_Configurator)} selected="selected"{/if}>{$lng.lbl_disabled}</option>
...
{include file="main/clean_url_field.tpl" clean_url=$product.clean_url clean_urls_history=$product.clean_urls_history clean_url_fill_error=$top_message.clean_url_fill_error}
...
<div{if $active_modules.HTML_Editor and !$html_editor_disabled} class="description"{/if}>{$lng.lbl_short_description}* :</div>
...
<tr> 
  ...
  <td colspan="2" class="FormButton">
<div{if $active_modules.HTML_Editor and !$html_editor_disabled} class="description"{/if}>{$lng.lbl_det_description}* :</div>
<div class="description-data">
  ...
  {if $top_message.fillerror ne "" and $product.xss_fulldescr eq "Y"}<font class="Star">&lt;&lt;</font>{/if}
</div>
  </td>
</tr>
...
<tr>
  ...
  <td class="ProductDetails"><textarea name="title_tag" cols="45" rows="12" class="InputWidth">{$product.title_tag}</textarea></td>
</tr>
...
<tr>
  ...
  <td class="ProductDetails"><textarea name="meta_keywords" cols="45" rows="12" class="InputWidth">{$product.meta_keywords}</textarea></td>
</tr>
...
<tr>
  ...
  <td class="ProductDetails"><textarea name="meta_description" cols="45" rows="12" class="InputWidth">{$product.meta_description}</textarea></td>
</tr>
...
<tr>
  ...
  <tr>
    ...
    <td align="center" width="100%">{if $new_product eq 1}&nbsp;{else}<a href="javascript:void(0);" onclick="javascript: window.open('unavailable_shipping.php?id={$product.productid}', '','width=500,height=350,toolbar=no,status=no,scrollbars=yes,resizable=no,menubar=no,location=no,direction=no');">{$lng.lbl_check_for_unavailable_shipping_methods}</a>{/if}</td>
  </tr>
  </table>
  </td>
</tr>
...
<tr>
  ...
  <td class="ProductDetails">
  <input type="text" name="items_per_box" size="18" value="{$product.items_per_box|default:1}"{if $product.small_item eq "Y" || $product.separate_box ne "Y"} disabled="disabled"{/if} />
  </td>
</tr>
...
<tr> 
  ...
  {if $usertype eq "P" or $active_modules.Simple_Mode ne ""}<br /><a href="{$catalogs.provider}/taxes.php" class="SmallNote" target="_blank">{$lng.lbl_click_here_to_manage_taxes}</a>{/if}
  </td>
</tr>
{/if}
