{*
$Id: button.tpl,v 1.14.2.1 2009/12/22 09:09:44 aim Exp $
...
{elseif $style eq 'image'}
  ...
  {if $type eq 'input'}
    ...
    <input class="image-button{if $additional_button_class} {$additional_button_class}{/if}" type="image" src="{$ImagesDir}/spacer.gif" alt="{$title|default:$button_title|escape}"{if $js_link} onclick="{$js_link}"{/if} />
...
{elseif $style eq 'dropout'}
  ...
  <div class="dropout-wrapper">
    <div class="dropout-container">
      ...
      {/if}
      <!--[if lte IE 6.5]><iframe src="{$ImagesDir}/spacer.gif"></iframe><![endif]-->
      </div>
    </div>
  </div>
...
{elseif $style eq 'div_button'}
  ...
  <div class="button{if $additional_button_class} {$additional_button_class}{/if}" title="{$title|default:$button_title|escape}"{if $js_link} onclick="{$js_link}"{/if}>
    <div{$reading_direction_tag}>{$button_title|amp}</div>
  </div>

{else}

  <button class="button{if $additional_button_class} {$additional_button_class}{/if}" type="{if $type eq 'input'}submit{else}button{/if}" title="{$title|default:$button_title|escape}"{if $js_link} onclick="{$js_link}"{/if}>
  {strip}
