{*
$Id: cart_contents.tpl,v 1.39 2009/11/02 14:28:02 igoryan Exp $
...
<table cellspacing="1" class="cart-content width-100" summary="{$lng.lbl_products|escape}">
  ...
  {foreach from=$products item=product name=products}
    ...
    <tr{interline index=$smarty.foreach.products.index total=$list_length}>
      ...
      <td class="cart-column-price cart-content-text">{include file="currency.tpl" value=$product.display_price display_sign=$product.price_show_sign}</td>
      <td class="cart-content-text">
        {if $config.Appearance.allow_update_quantity_in_cart eq "N" || ($active_modules.Egoods && $product.distribution) || ($active_modules.Subscriptions && $product.sub_plan) || ($active_modules.Product_Configurator && $product.hidden) || $link_qty eq "Y"}
          {$product.amount}
