{*
$Id: search_result.tpl,v 1.81 2009/07/08 12:18:07 joy Exp $
vim: set ts=2 sw=2 sts=2 et:
*}
{if $mode ne "search" || $products eq ""}
    ...
    <form name="searchform" action="search.php" method="post">
      ...
      <table cellspacing="0" cellpadding="0" class="width-100" summary="{$lng.lbl_search_products|escape}">
        <tbody>
          <tr>
            ...
            <tr>
              ...
              <td colspan="2" class="input-row">
                ...
                <label>
                  <input type="radio" name="posted_data[including]" value="all"{if $is_empty_search_prefilled || $search_prefilled.including eq '' || $search_prefilled.including eq 'all'} checked="checked"{/if} />
                  {$lng.lbl_all_word}
                </label>
          ...
          <tr>
            ...
            <td class="input-row" colspan="2">
              ...
              <label>
                <input type="checkbox" name="posted_data[by_title]"{if $is_empty_search_prefilled || $search_prefilled.by_title} checked="checked"{/if} />
                {$lng.lbl_product_title}
              </label>

              <label>
                <input type="checkbox" id="posted_data_by_descr" name="posted_data[by_descr]"{if $is_empty_search_prefilled || $search_prefilled.by_descr} checked="checked"{/if} />
                {$lng.lbl_description}
              </label>
        ...
        {if $config.Search_products.search_products_category eq 'Y' || ($active_modules.Manufacturers && $config.Search_products.search_products_manufacturers eq 'Y') || $config.Search_products.search_products_price eq 'Y' || $config.Search_products.search_products_weight eq 'Y'}
          ...
          <tbody id="adv_search_box"{if $js_enabled eq 'Y' && !$search_prefilled.need_advanced_options} style="display: none;"{/if}>
            ...
            {if $config.Search_products.search_products_category eq 'Y'}
                ...
                <td class="data-input" colspan="2">
                  <select name="posted_data[categoryid]" class="adv-search-select">
                    ...
                    {foreach from=$search_categories item=v}
                      <option value="{$v.categoryid}"{if $search_prefilled.categoryid eq $v.categoryid} selected="selected"{/if}>{if $config.UA.browser eq "MSIE"}{$v.category_path|truncate:60:'...':true:true|amp}{else}{$v.category_path|amp}{/if}</option>
                    {/foreach}
                  </select>
                </td>
              </tr>
              ...
              <tr>
                ...
                <td colspan="2">
                  ...
                  <label>
                    <input type="checkbox" name="posted_data[search_in_subcategories]"{if $is_empty_search_prefilled || $search_prefilled.search_in_subcategories} checked="checked"{/if} />
