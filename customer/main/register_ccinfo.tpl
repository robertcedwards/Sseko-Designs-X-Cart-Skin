{*
$Id: register_ccinfo.tpl,v 1.12 2009/09/16 07:10:19 max Exp $
...
{if $config.General.check_cc_number eq "Y" and ($payment_cc_data.type eq "C" or $payment_data.paymentid eq 1 or  ($payment_data.processor_file eq "ps_paypal_pro.php" and $payment_cc_data.paymentid ne $payment_data.paymentid)) and $payment_cc_data.disable_ccinfo ne "Y"}
  ...
  {if $payment_cc_data.disable_ccinfo ne "C" && $payment_cc_data ne ""}
...
<tr>
  ...
  <td>
    ...
    {if #safeCCNum# eq ""}
      ...
      {if $smarty.get.err eq 'fields' && $userinfo.card_type eq ''}
...
<tr>
  ...
  <td>
    ...
    {if #safeCCNum# eq ""}
      ...
      {if $smarty.get.err eq 'fields' && $userinfo.card_name eq ''}
        <span class="error-message">&lt;&lt;</span>
      {/if}
...
<tr>
  ...
  <td>
    ...
    {if #safeCCNum# eq ""}
      <input type="text" name="card_number[{unique_key}]" size="32" maxlength="20" value="{if $smarty.get.err eq 'fields'}{$userinfo.card_number}{elseif $userinfo.card_number_w ne '' && $payment_data.paymentid ne ''}{$userinfo.card_number_w}{/if}" />

      {if $smarty.get.err eq 'fields' && $userinfo.card_number eq ''}
        <span class="error-message">&lt;&lt;</span>
      {/if}
...
{if $payment_cc_data.disable_ccinfo eq "N" || ($payment_cc_data.disable_ccinfo eq "" && $config.General.enable_manual_cc_cvv2 eq 'Y')}
  {foreach from=$card_types item=ct}
  {if $userinfo.card_type eq $ct.code && $ct.cvv2 eq ''}{assign var='is_cvv2_required' value='N'}{/if}
  {/foreach}
  <tr>
    <td class="data-name">{$lng.lbl_cc_cvv2}</td>
    <td class="data-required" id="cvv2_star">{if $is_cvv2_required eq 'Y'}*{/if}</td>
    <td class="valign-middle">

      {if #safeCCNum# eq ""}
        <input type="text" name="card_cvv2[{unique_key}]" size="4" maxlength="4" value="{if $smarty.get.err eq 'fields'}{$userinfo.card_cvv2}{elseif $userinfo.card_cvv2_w ne '' && $payment_data.paymentid ne ''}{$userinfo.card_cvv2_w}{/if}" />
