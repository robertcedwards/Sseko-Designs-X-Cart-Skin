{*
$Id: orders_list.tpl,v 1.8 2009/09/07 07:52:02 igoryan Exp $
      ...
      {foreach from=$orders item=order name=orders}
        ...
        <tr{interline name=orders}>
          ...
          <td>
            ...
            {if $active_modules.Stop_List && $order.blocked  eq 'Y'}
              <img src="{$ImagesDir}/spacer.gif" class="slist-no-ip" alt="{$lng.lbl_blocked}:{$order.ip}" title="{$lng.lbl_ip_blocked|substitute:"ip":$order.ip}" />
            {/if}
          </td>
          <td><a href="order.php?orderid={$order.orderid}">{$order.date|date_format:$config.Appearance.datetime_format}</a></td>
          <td class="data-right-column"><a href="order.php?orderid={$order.orderid}">{include file="currency.tpl" value=$order.total}</a></td>
        </tr>

      {/foreach}

      <tr>
        <td colspan="5" class="data-right-column">{$lng.lbl_gross_total}: <strong>{include file="currency.tpl" value=$total}</strong></td>
      </tr>

      <tr>
        <td colspan="5" class="data-right-column">{$lng.lbl_total_paid}: <strong>{include file="currency.tpl" value=$total_paid}</strong></td>
      </tr>
