{*
$Id: register.tpl,v 1.52 2009/06/26 11:53:15 joy Exp $
...
<form action="{$register_script_name}?{$smarty.server.QUERY_STRING|amp}" method="post" name="registerform"{if $js_enabled} onsubmit="javascript:if (check_zip_code(){if $default_fields.email.required eq 'Y'}&amp;&amp;checkEmailAddress(document.registerform.email){/if}&amp;&amp;checkRequired(requiredFields, false, {if $is_admin_editor}true{else}false{/if}){if $config.Security.use_complex_pwd eq 'Y'}&amp;&amp;checkPasswordStrength(document.registerform.passwd1,document.registerform.passwd2){/if}) return true; else return false;"{/if}>
