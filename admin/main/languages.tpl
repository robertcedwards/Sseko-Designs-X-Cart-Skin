{*
$Id: languages.tpl,v 1.64 2009/10/07 07:10:20 igoryan Exp $
    ...
    <select name="language" onchange='javascript: self.location="languages.php?language="+this.value;'>
      <option value=""{if $smarty.get.language eq ""} selected="selected"{/if}>{$lng.lbl_select_one}</option>
        {foreach from=$languages item=l}
          <option value="{$l.code|escape}"{if $smarty.get.language eq $l.code} selected="selected"{/if}>{$l.language}</option>
        {/foreach}
    </select>
  </td>
  {if $smarty.get.language ne "" and $smarty.get.language ne $shop_language}
    <td>
      <input type="button" value="{if $language_data.disabled eq "Y"}{$lng.lbl_enable|strip_tags:false|escape}{else}{$lng.lbl_disable|strip_tags:false|escape}{/if}" onclick="javascript: self.location='languages.php?language={$smarty.get.language|escape:html}&amp;mode=change';" />
      <input type="button" value="{$lng.lbl_delete|strip_tags:false|escape}" onclick="javascript: if (confirm(txt_are_you_sure)) self.location='languages.php?language={$smarty.get.language|escape:html}&amp;mode=del_lang';" />
    </td>
  {/if}
</tr>
...
var delete_link = 'languages.php?mode=delete&page={$page}&language={$smarty.get.language}&filter={$filter}&topic={$topic}&var=';
