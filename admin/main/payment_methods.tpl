{*
$Id: payment_methods.tpl,v 1.62 2009/10/29 11:02:57 max Exp $
...
{if $recent_payment_methods}
  ...
  {capture name=dialog}
      ...
      {foreach from=$recent_payment_methods item=rpm name=rpm}
        ...
        <tr{interline class="TableSubHead" name=rpm}>
          ...
          <td style="padding-left: 20px;">
            <a href="javascript:void(0);" class="NeedHelpLink" onmouseover="javascript: viewHelp('help_what_is_webbased', this, 500, 400);">
    ...
    <div id="help_what_is_webbased" class="NeedHelpBox" style="display:none">{$lng.txt_payment_methods_types}</div>
    {if $has_webbased && $smarty.foreach.rpm.total gt 4}
...
{capture name=dialog}
  ...
  <form action="payment_methods.php" method="post" name="pmform">
    ...
    <tr class="TableHead">
      ...
      <th width="20%" nowrap="nowrap">{$lng.lbl_special_instructions}</th>
    ...
    {section name=method loop=$payment_methods}
      ...
      {/if}
      <td valign="top"{if $payment_methods[method].module_name ne ""} rowspan="2"{/if}>
        <input type="text" size="5" maxlength="5" name="posted_data[{$payment_methods[method].paymentid}][orderby]" value="{$payment_methods[method].orderby}"{if $payment_methods[method].disable_checkbox} disabled="disabled"{/if}{if $payment_methods[method].control_checkbox} onchange="javascript: changeDisabledOrderBy(this);"{/if} />
      </td>
    </tr>
      ...
      <tr{if $payment_methods[method].active eq "Y"}{$active_trcolor}{else}{$inactive_trcolor}{/if}>
        <td colspan="3" valign="bottom">
          ...
          {if not $payment_methods[method].disable_checkbox}
            ...
            {if $payment_methods[method].is_down || $payment_methods[method].in_testmode}
              <table cellpadding="2">
...
<script type="text/javascript" src="{$SkinDir}/admin/main/payment_methods.js"></script>
