{*
$Id: pages.tpl,v 1.27 2009/08/28 07:31:16 igoryan Exp $
...
<td width="5"><input type="checkbox" name="to_delete[{$pages[pg].pageid}]" value="Y" /></td>
...
<td align="center"><input type="checkbox" name="posted_data[{$pages[pg].pageid}][show_in_menu]" value="Y"{if $pages[pg].show_in_menu eq 'Y'} checked="checked"{/if} /></td>
...
<td align="right" width="30"><a href="{$catalogs.customer}/pages.php?pageid={$pages[pg].pageid}&amp;mode=preview" target="previewpage">{$lng.lbl_preview}</a></td>
</tr>
...
<tr{cycle name="root" values=", class='TableSubHead'"}>
  ...
  <td><b><a href="pages.php?pageid={$pages[pg].pageid}" title="{$pages[pg].filename|escape}">{$pages[pg].title|truncate:"30":"..."}</a></b></td>
  <td align="right" width="30"><a href="{$xcart_web_dir}/{$pages[pg].filename}" target="previewpage">{$lng.lbl_preview}</a></td>
</tr>
...
<tr>
<td colspan="3"><br /><br />{include file="main/subheader.tpl" title=$lng.lbl_root_level class="grey"}</td>
</tr>
...
<tr class="TableHead">
<td width="2">&nbsp;</td>
<td colspan="2">{$lng.lbl_page_title}</td>
</tr>
...
<tr>
<td align="center" colspan="3">{$lng.txt_no_root_pages}</td>
</tr>
