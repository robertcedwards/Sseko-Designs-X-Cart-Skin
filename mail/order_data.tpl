{*
$Id: order_data.tpl,v 1.49 2009/06/04 13:22:59 igoryan Exp $
...
{/if}
{$lng.lbl_item_price|mail_truncate}{include file="currency.tpl" value=$products[prod_num].display_price display_sign=$product.extra_data.price_show_sign}
...
{if $tax.tax_value gt 0}{$tax.tax_display_name} {if $tax.rate_type eq "%"}{$tax.rate_value}%{else}{include file="currency.tpl" value=$tax.rate_value}{/if}{/if}
...
{$lng.lbl_amount|mail_truncate}{include file="currency.tpl" value=$giftcerts[giftcert].amount}
...
{$lng.lbl_delivery|mail_truncate}{$order.shipping|trademark}
{$lng.lbl_subtotal|mail_truncate}{include file="currency.tpl" value=$order.display_subtotal}

{if $order.discount gt 0}{$lng.lbl_discount|mail_truncate}{include file="currency.tpl" value=$order.discount}{/if}

{if $order.coupon and $order.coupon_type ne "free_ship"}
{$lng.lbl_coupon_saving|mail_truncate}{include file="currency.tpl" value=$order.coupon_discount} ({$order.coupon})
{/if}
{if $order.discounted_subtotal ne $order.subtotal}
{$lng.lbl_discounted_subtotal|mail_truncate}{include file="currency.tpl" value=$order.display_discounted_subtotal}

{/if}
{$lng.lbl_shipping_cost|mail_truncate}{if $order.coupon and $order.coupon_type eq "free_ship"}{include file="currency.tpl" value=0}{else}{include file="currency.tpl" value=$order.display_shipping_cost}{/if}

{if $active_modules.Gift_Registry and $order.need_giftwrap eq "Y"}
{$lng.lbl_giftreg_gift_wrapping|mail_truncate}{include file="currency.tpl" value=$order.giftwrap_cost}
{/if}
...
{if $tax.rate_type eq "%"}{assign var="rate_value" value=$tax.rate_value}{assign var="tax_display_name" value="`$tax.tax_display_name` `$rate_value`%"}{else}{assign var="tax_display_name" value=$tax.tax_display_name}{/if}{$tax_display_name|mail_truncate}{include file="currency.tpl" value=$tax.tax_cost}

{/foreach}
{/if}
{if $order.payment_surcharge ne 0}
{if $order.payment_surcharge gt 0}{$lng.lbl_payment_method_surcharge|mail_truncate}{else}{$lng.lbl_payment_method_discount|mail_truncate}{/if}{include file="currency.tpl" value=$order.payment_surcharge}
{/if}
{if $order.giftcert_discount gt 0}
{$lng.lbl_giftcert_discount|mail_truncate}{include file="currency.tpl" value=$order.giftcert_discount}
{/if}

{$lng.lbl_total|mail_truncate}{include file="currency.tpl" value=$order.total}
...
{section name=gc loop=$order.applied_giftcerts}
    {$order.applied_giftcerts[gc].giftcert_id|mail_truncate}{include file="currency.tpl" value=$order.applied_giftcerts[gc].giftcert_cost}
