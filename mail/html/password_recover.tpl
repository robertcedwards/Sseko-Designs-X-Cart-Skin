{*
$Id: password_recover.tpl,v 1.12 2009/09/01 12:07:04 igoryan Exp $
...
<td>&nbsp;</td>
<td><tt><a href="{if $config.Security.use_secure_login_page eq 'Y'}{$https_location}{else}{$http_location}{/if}{if $userpath ne ''}{$userpath}{/if}/change_password.php?password_reset_key={$account.password_reset_key}&amp;username={$account.login}">{if $config.Security.use_https_login eq 'Y'}{$https_location}{else}{$http_location}{/if}{if $userpath ne ''}{$userpath}{/if}/change_password.php?password_reset_key={$account.password_reset_key}&username={$account.login}</a></tt></td>
</tr>
</table>
