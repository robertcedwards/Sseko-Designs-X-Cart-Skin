{*
$Id: register_provider.tpl,v 1.12 2009/09/01 12:07:05 igoryan Exp $
...
{if $config.General.allow_change_seller_address ne "Y" and $main ne "user_profile"}
...
<form action="{$register_script_name}?{$smarty.server.QUERY_STRING|amp}" method="post" name="registerform"{if $js_enabled} onsubmit="javascript: if (check_zip_code() && checkRequired(requiredFields)) return true; else return false;"{/if}>
...
{if ($reg_error ne "" and $default_fields.state.required eq 'Y' and $userinfo.seller_state eq "" && $userinfo.s_display_states) or $error eq "statecode"}<font class="Star">&lt;&lt;</font>{/if}
</td>
</tr>
